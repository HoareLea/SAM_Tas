...
{
    public class TasTSDQueryZoneResultsByPercentage : GH_SAMVariableOutputParameterComponent
    {
        /// <summary>
        /// Gets the unique ID for this component. Do not change this ID after release.
        /// </summary>
        public override Guid ComponentGuid => new Guid("aa393cdf-cb1a-4661-83db-d5a6bcb3e406");
            ...
            if (spaces_AdjacencyCluster != null || spaces_AdjacencyCluster.Count != 0)
            {
                for (int a = 0; a < zones.Count; a++)
                {
                    ...
                    foreach (Space space in spaces_Zone)
                    {
                        ...
                        {
                            double value_Temp = (double)jArray[j];
                            if (dictionary.ContainsKey(j))
                            {
                                dictionary[j] += value_Temp;
                            }
                            else
                            {
                                dictionary[j] = value_Temp;
                    ...
                    foreach (double value_Temp in dictionary.Values)
                    {
